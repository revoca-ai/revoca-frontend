...
const links: { num: string; label: string; href: string; external?: boolean }[] = [
  ...
  { num: "03", label: "Integrations", href: "#integrations" },
  { num: "04", label: "Docs", href: "https://notion.so/revoca-ai", external: true },
];
...
export default function Nav() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[100] bg-[rgba(5,5,5,0.85)] backdrop-blur-xl border-b border-[#151515]">
      <div className="flex items-center justify-between px-4 sm:px-6 lg:px-[100px] py-4 sm:py-5">
        ...
        <button
          ...
          aria-label="Menu"
        >
          <span className={`w-5 h-px bg-[#666] transition-all duration-300 ${open ? "rotate-45 translate-y-[3.5px]" : ""}`} />
          <span className={`w-5 h-px bg-[#666] transition-all duration-300 ${open ? "opacity-0" : ""}`} />
          <span className={`w-5 h-px bg-[#666] transition-all duration-300 ${open ? "-rotate-45 -translate-y-[3.5px]" : ""}`} />
        </button>
      </div>
      ...
      {/* Mobile menu */}
      {open && (
        <div className="md:hidden border-t border-[#111] px-6 py-6 flex flex-col gap-4 font-mono text-sm">
          {links.map((link) => (
            <a
              key={link.href}
              href={link.href}
